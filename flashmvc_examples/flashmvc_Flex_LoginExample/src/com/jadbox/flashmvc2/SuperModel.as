package com.jadbox.flashmvc2
{
	...
	public class SuperModel extends EventDispatcher
	{
		 ...
		 * @param listener The callback function for the SuperEvent (or extended event)
		 * @param superEventType The subtype of SuperEvent to only listen for (will not detect further subclassed events). This allows the view to monitor several actions that take the same event. (default is SuperEvent)
		 * 
		 */		
		public function addEventsListener(type:String, listener:Function, superEventType:Class=null):void {
			if(!superEventType) superEventType = SuperEvent;
			var actions:Array = superActions;
			for(var i:int=0; i < actions.length; i++) {
				var action:SuperAction = this[actions[i]];
				if ( action.eventRef == superEventType) action.addEventListener(type, listener);
			}
		}
		
		public function syncVars(...VARS):void {
